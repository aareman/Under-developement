...
Plot.prototype.tpRecord = function(variable, graph,res=1){
	//if(this.tres_counter >= res){
		this.tres_counter = 1; //tres = time resoloution
		this.timeplot = true;
...
function Graph(w, h, x_min, x_max, y_min, y_max, resoloution){
	...
	this.bl_pix = new Point(this.x_offset+this.width*0.15,
								this.y_offset+this.height*0.90);
...
Graph.prototype.drawBg = function(bg = color(255), border = color(0)){
	...
	var pixCount = 0;
		//draw min label	
		text(this.x_min, this.bl_pix.x, this.bl_pix.y+16);
	for(var i = 0; i < this.resoloution; i++){
		count += xdiff;
		pixCount += this.xpix;
		line(this.bl_pix.x + pixCount, this.bl_pix.y+5, 
				this.bl_pix.x + pixCount, this.tr_pix.y);
		text((Math.round(10*count)/10).toString(), this.bl_pix.x + pixCount, this.bl_pix.y+16);		
	}
	...
	var pixCount = 0;
		//draw min label	
		text(this.y_min, this.bl_pix.x - textWidth(this.y_min.toString()), this.bl_pix.y);
	for(var i = 0; i < this.resoloution; i++){
		count += ydiff;
		pixCount += this.ypix;
		line(this.bl_pix.x - 5, this.bl_pix.y-pixCount, 
				this.tr_pix.x, this.bl_pix.y-pixCount);
		text((Math.round(10*count)/10).toString(), this.bl_pix.x - textWidth((Math.round(10*count)/10).toString()), this.bl_pix.y-pixCount);		
	}
	...
	if(this.showTitle == true){
		...
		if(this.showLegend != true){
			...
			for(var i=0;i<this.plots.length;i++){
				if(this.plots[i].show == true){
					text(this.plots[i].plotTitle, left+8,top+2);
					stroke(this.plots[i].color);
					line(left+8+textWidth(this.plots[i].plotTitle)+4,
						top+8, left+this.width/2.4, top+8);
	...
	if(this.showLabels == true){
		...
		//ylabel
		text(this.ylabel,this.x_offset+this.width*.04,this.y_offset + this.height/2);
	}
	pop();	
};
